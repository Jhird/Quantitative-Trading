...
-example-
python marketsim_IBB.py 1000000 orders.csv values.csv
Inputs:
...
Output:
    - Performance of portfolio against IBB index (Biotech Only)
...
Location = os.getcwd()
orders_file = 'TS_bollinger_bands_orders.csv'#str(sys.argv[2])
...
## Load Data for all years (Adj_Close from 2011 to 2015)
Adj_Close_ALL = []

for i in range(2011,2016):
    filename = 'Adj_Close' + '_IBB_' + str(i) + '.csv'
    filename = os.getcwd() + '\\' + filename
    Adj_Close = read_csv(filename, index_col=0) # Read QSTK compatible dataframe
    Adj_Close_ALL.append(Adj_Close)

Adj_Close_ALL = pd.concat(Adj_Close_ALL)
...
# Initialize Cash
init_cash = 100000.0#float(str(sys.argv[1]))#1000000.0
c = init_cash
...
filename = 'TS_bollinger_bands_values.csv'#str(sys.argv[3])
f = open(filename,'w')
for i in range(len(ldt_timestamps)):
...
# GET PRICES OF IBB
market_prices = pd.DataFrame(data=np.array(Adj_Close_ALL['IBB'].loc[trade_dates]),index = ldt_timestamps,columns=['Market_Price'])
...
f.write("Sharpe Ratio of Market : %f \n" % sharpe_market)
f.write("Total Return of Fund : %f " % cum_ret)
f.write("Total Return of Market : %f \n" % cum_ret_market)
f.write("Standard Deviation of Fund : %f " % vol)
f.write("Standard Deviation of Market : %f \n" % vol_market)
f.write("Average Daily Return of Fund : %f " % daily_ret_avg)
f.write("Average Daily Return of Market : %f \n" % daily_ret_avg_market)
...
plt.legend(['Portfolio','Benchmark'+' (IBB)'])
